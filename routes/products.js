...
router.get("/", async (req, res, next) => {
  try {
    const { page, limit, sortOrder, categoryId, q } = req.query;
    const allPro = await productController.getAll(page, limit, sortOrder,categoryId,q);
...
router.post("/addProduct", upload.single("img"), async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ result, status: "OK", message: "Thêm Sản Phẩm Thành Công" });
  } catch (error) {
    ...
    try {
      ...
      if (!product) {
        return res
          .status(200)
          .json({
            message: "Sản Phẩm đang tồn tại trong đơn hàng, chỉ có thể ẩn ",
          });
      }
...
router.get("/cate/:categoryId", async (req, res) => {
  ...
  try {
    const allPro = await productController.productByCategoryId(
      categoryId,
      page,
      limit,
      sortOrder
